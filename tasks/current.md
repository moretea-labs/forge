...
- Source repair in progress: persist an explicit Connector instance identity (WSL target `forge-wsl`) through the Controller Home and service environment; bring the standalone Recovery installer into parity with the CLI for Linux/OpenAI Primary tunnel ownership and distinct tunnel identities.
