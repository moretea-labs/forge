...
- Architecture v2 gate alignment: the redundant Process Runtime case that expected `gh issue comment` to execute through an ordinary Process lane was retired; the remaining focused Process Runtime suite passes 77/77 and the lifecycle regression preserves `durable + not_started + never_auto_retry` without Process or Lease state. Stale exact Scheduler source-text assertions superseded by runtime architecture and behavior coverage were also retired; the focused Runtime command suite passes 6/6.
