...
const entry = join(root, 'src', 'runtime', 'root', 'entry.ts');
...
function run(command, commandArgs) {
  ...
  if (result.error) {
    console.error(`repo-harness-runtime launcher failed to run ${command}: ${result.error.message}`);
    process.exit(1);
  }
...
run(process.execPath, ['--loader', nodeLoader, entry, ...args]);
