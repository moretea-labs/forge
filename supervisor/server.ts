...
async function dispatch(control: WorkflowSupervisorControlPlane, req: RpcRequest): Promise<unknown> {
  ...
  if (req.method === 'browser_begin_effect') return control.browserBeginEffect({ conversationId: text(p, 'conversation_id'), conversationUrl: text(p, 'conversation_url'), effectId: text(p, 'effect_id'), dispatchId: text(p, 'dispatch_id'), evidence: object(p.evidence) });
