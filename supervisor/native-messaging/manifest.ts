...
export function renderWorkflowSupervisorNativeManifest(input: WorkflowSupervisorNativeManifestInput): string {
  ...
  return `${JSON.stringify({ name: WORKFLOW_SUPERVISOR_NATIVE_HOST_NAME, description: 'Stateless Chrome relay to the Forge Workflow Supervisor authority', path: input.executablePath, type: 'stdio', allowed_origins: [`chrome-extension://${input.extensionId}/`] }, null, 2)}\n`;
}
