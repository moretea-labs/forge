...
function usage(): never {
  throw new Error('Usage: bun scripts/install-desktop-operator-registration.ts --controller-home <path> --socket <absolute-path> [--version 0.1.0] [--protocol 1.0] [--expected-revision N] [--disabled]');
}

const controllerHome = argument('--controller-home') ?? process.env.FORGE_CONTROLLER_HOME?.trim();
const socketPath = argument('--socket');
if (!controllerHome || !socketPath) usage();
