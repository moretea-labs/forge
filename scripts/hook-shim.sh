#!/usr/bin/env bash
# scripts/hook-shim.sh — forge global hook dispatcher (Phase 0.5 bash prototype).
#
# Installed by `scripts/forge.sh install` to ~/.forge/hook-shim.sh.
# Phase 1 CLI replaces this file with `forge hook <event>` subcommand.
#
# Invoked from user-level hook configs:
#   bash ~/.forge/hook-shim.sh <hook-script-name>.sh [extra-args...]
#
# Behavior:
#   1. Resolve current repo via `git rev-parse --show-toplevel`
#   2. If not in a git repo OR not forge opt-in → silent exit 0
...
#   a. FORGE_HOOK_SOURCE env: `repo` | `central` | absolute hooks dir
...
#                `scripts/forge.sh trust|untrust|trust-list`.

set -euo pipefail

HOOK_NAME="${1:-}"
if [ -z "$HOOK_NAME" ]; then
  echo "[forge-shim] missing hook script name" >&2
  exit 2
fi

FORGE_HOME="${FORGE_HOME:-$HOME/.forge}"
TRUST_FILE="$FORGE_HOME/trusted-repos"
...
if ! { [ -f "$TRUST_FILE" ] && grep -Fxq "$primary_root" "$TRUST_FILE"; }; then
  if [ "$HOOK_NAME" = "session-start-context.sh" ]; then
    echo "[forge-shim] repo not trusted; hooks skipped. Trust it with: bash scripts/forge.sh trust $primary_root" >&2
  fi
  exit 0
fi

resolve_hooks_dir() {
  case "${FORGE_HOOK_SOURCE:-}" in
    repo) printf '%s' "$repo/.ai/hooks"; return ;;
    central) printf '%s' "$CENTRAL_HOOKS_DIR"; return ;;
    /*) printf '%s' "$FORGE_HOOK_SOURCE"; return ;;
  esac
