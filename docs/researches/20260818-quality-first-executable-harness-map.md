...
- Browser/session discovery should prefer the browser that actually carries the authenticated ChatGPT session; do not hard-code Chrome/Vivaldi or silently fall back to an anonymous managed profile when the authenticated browser is temporarily closed.
- Keep the GitHub-hosted Forge Cloud workflow manual-only as a bounded diagnostic fallback. Do not auto-renew the dedicated tunnel on a recurring workflow, and do not treat hosted Actions as permanent Forge Cloud hosting.
- The Google Cloud `forge-cloud` e2-micro experiment was retired on 2026-08-26 after the Secure Tunnel alternated between HTTP 404/429, direct SSH and IAP SSH failed, and WARP reported repeated main-loop watchdog hangs plus QUIC idle timeout. Do not restart or reuse that VM.
- Any future long-lived Cloud Source Maintainer must use a separately evaluated host/transport and must demonstrate materially better latency and reliability than the local Forge path before it becomes an execution authority.
