...
- Command failure, explicit close, or the next access after expiry attempts to close the provider session. Terminal state is written only after close succeeds; cleanup failure remains `closing`, keeps device ownership fenced, and can be retried through the idempotent close action. Fill arguments and diagnostics are redacted from failure evidence. The lightweight interaction record remains bounded, while the provider daemon and a healthy XCTest Runner use configurable finite idle retention (five minutes by default) so adjacent trusted actions do not repeatedly pay cold-start cost; explicit close still terminates ownership immediately.
