...
- Ordinary target commands remain typed-argv and classifier-bounded. A project script that cannot be proven read-only uses a separate high-risk action: the caller selects one fixed interpreter resolved from a closed set of canonical system locations, one root-contained script, and its exact SHA-256 digest, then provides strong confirmation. Caller-supplied executable paths, eval flags, and shell command strings are not accepted, and execution holds the target mutation lock with destructive authorization recorded on the Work contract.
