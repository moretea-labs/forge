...
**2026-09-16 authority clarification:** this `turn settled` fact is a lower-layer ControllerRound liveness fallback only. It does not prove that a ChatGPT Web assistant response committed the Workflow Supervisor completion protocol, and it does not own cross-assistant-turn scheduling after the caller has ended. The outer commit/start authority is defined by [`20260916-workflow-supervisor-authority.md`](20260916-workflow-supervisor-authority.md), which requires the complete terminal control block and exact END marker plus causal effect identity.
