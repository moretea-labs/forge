...
This foundation supplies memory and learning primitives, not uncontrolled self-modification. Strategy evolution follows evidence-backed candidate -> shadow/canary/A-B -> promotion receipt. The promotion receipt is a deterministic, immutable evaluation attestation, not lifecycle or mutation authority: it is minted only by re-evaluating the existing frozen paired A/B and operational-shadow evidence (and, when supplied for release-level changes, the existing V2 certification manifest), and any regressed/inconclusive blocking evidence prevents minting. The receipt has no clock-driven state, store, scheduler, policy writer, or source writer. Architecture-level recurring patterns therefore enter the existing lifecycle by carrying the receipt id as a normal `Requirement.requiredDeliveryReferences` item; Requirement/Plan/Work, review, verification and human/Controller semantic acceptance remain the only change authorities.
