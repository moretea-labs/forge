...
One immutable release identity binds the stable `forge-runtime` supervisor launcher, the manifest-attested Runtime interpreter/bundle pair, and every bounded child the Canonical Runtime launches:
...
The OS service manager starts and automatically restarts one stable `forge-runtime` supervisor launcher after unexpected service death. The service executes from the stable physical path `ControllerHome/runtime/service/active-forge-runtime`; that file is an atomic byte-for-byte mirror of the signed launcher in the selected immutable release, not a symlink into `runtime/releases/<releaseId>`. The launcher validates the manifest-bound Runtime bundle plus the co-located release-owned Bun interpreter, starts exactly one interpreter child, forwards lifecycle and diagnostic signals, waits for that child, and mirrors its abnormal terminal outcome. The launcher carries no Scheduler, Gateway, Work, or release-transition authority and never imports the long-lived Runtime bundle into its compiled JSC VM. Release assets and semantic identity remain bound by the selected manifest and `FORGE_RELEASE_*` environment. The fixed physical launcher path remains the macOS TCC responsible-process boundary inherited by Runtime descendants across releases instead of creating one permission principal per release directory. Module readiness failure does not create component restart authority. A release transition stops and starts the complete Runtime service, refreshes the stable launcher mirror from the selected release before launch, verifies whole-Runtime readiness through the child-owned Runtime status/owner record, and on failure restores the previous complete release together with its matching stable mirror and bound SQLite backup.
...
The immutable release carries every execution dependency needed by the Runtime and its bounded children. This includes the exact Bun interpreter that hosts the long-lived Runtime bundle, the Process/Check runners, Browser/Desktop helpers, external-plugin probe, and the matching CodeGraph Node executable, read-only sidecar, and compiled library tree. The Runtime interpreter is a hashed executable artifact and release-canary target, not a PATH-resolved host dependency or separate lifecycle authority. CodeGraph is invoked once per bounded context query by the Runtime; it is not a daemon, service, recovery owner, readiness authority, or persistent state writer. Its only durable input is the repository-owned `.codegraph/` index selected for the request.
...
- unexpected launcher death is handled by the OS service manager; unexpected Runtime-child termination is mirrored by the launcher so the same service restart policy applies;
