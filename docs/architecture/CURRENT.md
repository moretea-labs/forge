...
`computer.console.unlock.v1` is a protected Computer capability, not a widening of `computer.input.v1`. Forge owns its provider-neutral contract, provider selection/negotiation, and explicit single-use authorization boundary; Desktop Operator owns macOS lock-state observation, exact `loginwindow` binding, bounded physical input, attempt limiting, and authoritative locked-to-unlocked postcondition verification. The credential exists only in the protected request-local invocation path. ChatGPT-facing execution uses the dedicated `computer_console_unlock` Runtime tool, which runs synchronously on the canonical Runtime direct-control boundary and passes the credential straight to `executeRuntimeComputerConsoleUnlock`; it is never routed through generic Assistant Plugin actions, Process/Work/ExecutionJob state, receipts, evidence, or MCP audit hashing. Canonical Runtime proxy replay is intentionally ineligible, so a transport failure after dispatch is not retried. Results are an allowlisted postcondition projection and can never echo credential material. Ordinary `desktop_key`, `desktop_type_text`, batch, pointer, screenshot, application activation, shell, and compatibility paths cannot target the lock screen through this capability.
