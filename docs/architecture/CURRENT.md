...
- The default 19-tool MCP schemas are a deployment ABI because approved ChatGPT apps may retain a frozen action/input snapshot. Core quality capabilities therefore need a path through already-stable facade fields. `rh_context` supports precise `@tsnav` and `@swiftnav` directives in `query` as compatibility paths; refreshed clients may use the existing structured `semantic_navigation` field. When a newer canonical facade requires a bounded field that a frozen client cannot express, an existing stable compatibility carrier such as `capability_id` may transport only that missing typed field. The compatibility envelope is never semantic or persistence authority: decoded input must re-enter the same canonical Plan/Work admission and validation path, conflicting native+compatibility inputs are rejected, and malformed or incomplete obligation coverage remains blocked.
