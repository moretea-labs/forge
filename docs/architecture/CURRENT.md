...
8. `wait` keeps the semantic goal resumable. A later wake resumes the exact active Work when appropriate, or a successor Work/Plan step under the same Requirement/Plan lineage when the previous Work is terminal. `goal_complete` terminates the semantic goal and its continuation lane.
