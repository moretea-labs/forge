...
`computer.console.unlock.v1` is a protected Computer capability, not a widening of `computer.input.v1`. Forge owns its provider-neutral contract, provider selection/negotiation, and explicit single-use authorization boundary; Desktop Operator owns macOS lock-state observation, exact `loginwindow` binding, bounded physical input, attempt limiting, and authoritative locked-to-unlocked postcondition verification. Raw macOS credentials never enter ChatGPT, MCP arguments, or Forge Runtime. `computer_console_unlock_prepare` runs synchronously on the canonical Runtime direct-control boundary while the console is unlocked and asks Desktop Operator to collect the credential in a provider-local secure field; only a short-lived opaque single-use handle returns to Forge. `computer_console_unlock` later consumes that handle while the console is locked. Neither operation is routed through generic Assistant Plugin actions, Process/Work/ExecutionJob state, receipts, evidence, or MCP audit hashing, and Canonical Runtime proxy replay is intentionally ineligible. Results are allowlisted projections and cannot echo credential material. Ordinary `desktop_key`, `desktop_type_text`, batch, pointer, screenshot, application activation, shell, and compatibility paths cannot target the lock screen through this capability.
