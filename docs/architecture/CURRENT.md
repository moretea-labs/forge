...
- The default 19-tool MCP schemas are a deployment ABI because approved ChatGPT apps may retain a frozen action/input snapshot. Core quality capabilities therefore need a path through already-stable facade fields. `rh_context` supports precise `@tsnav` and `@swiftnav` directives in `query` as compatibility paths; refreshed clients may use the existing structured `semantic_navigation` field. For a newer semantic facade operation that an old `rh_work` operation enum cannot express, `operation=repair` plus `capability_id=semantic.v1:<bounded-payload>` is the generic transport-only compatibility envelope. The server owns an explicit per-operation allowlist and exact payload schema; the current allowlist admits only `requirement_create`, keeps `requirement_id` explicit outside the envelope, rejects unknown operations/fields/conflicting native inputs, and dispatches into the same canonical Requirement admission handler. Existing field-only carriers such as `plan.obligations.v1` remain bounded migration debt for deployed frozen clients and likewise re-enter canonical Plan validation. No compatibility decoder owns semantic decisions or persistence. The generic envelope is removed once supported clients provide guaranteed schema/version negotiation rather than per-conversation frozen snapshots; until then new schema gaps must extend the allowlisted generic transport instead of minting one bespoke lifecycle/persistence path per field.
