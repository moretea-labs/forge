...
- **One authority, many executions.** Requirement owns business lifecycle; a primary Work owns one objective-level continuity lane; repository commands, checks, managed processes, and typed plugin actions execute through their dedicated authorities rather than a second generic `work_submit` authority. A primary Work has one authoritative continuation Schedule; changing cadence or prompt updates that Schedule rather than creating another Workflow.
