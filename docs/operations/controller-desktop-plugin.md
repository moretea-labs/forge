...
Stable Supervisor releases include `repo-harness-desktop-helper.mjs` as a required, hashed release artifact. The Desktop adapter resolves that sibling artifact from the validated `REPO_HARNESS_RELEASE_PATH` identity injected into managed children, then uses argv/source candidates only as development compatibility fallbacks. It launches the helper with the same trusted Node resolver used by the bounded Node bridge path; it does not execute the compiled Repo Harness binary as a JavaScript interpreter. Release staging fails closed when the helper is missing or empty, and plugin health reports both helper availability and trusted runtime availability without returning either local path.
