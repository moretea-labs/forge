...
- `.forge/*.local.json`, tokens, OAuth, grants, plugin runtime JSON
...
`.gitignore` and `.npmignore` exclude runtime/plugin grants, tokens, `_ops`, research/tasks/plans noise, and key material patterns. Re-run the audit after adding new paths under `.forge/` or docs with machine-local examples.
