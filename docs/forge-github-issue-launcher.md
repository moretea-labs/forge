# Forge GitHub Integration

GitHub is an optional first-party Forge provider for repository collaboration and remote Issue/Project or pull-request operations. Local Requirement, Plan, Work, Process, Git, policy, evidence, and Runtime authority remain inside Forge; GitHub records are external collaboration artifacts, not a second lifecycle authority.

Remote writes require explicit authorization and idempotent provider evidence. Configure the provider through the Forge CLI or plugin action surface and verify repository mapping before mutation.

See [Integrations](wiki/Integrations.md), [Plugin baseline](architecture/CURRENT.md), and [Verification and Release Gates](architecture/CURRENT.md).
