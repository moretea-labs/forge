...
export function ensureLegacyBrowserSessionsImported(controllerHome: string, repoId: string, repoRoot: string): number {
  ...
  if (readControlPlaneRecord<LegacyImportMarker>(controllerHome, IMPORT_NAMESPACE, repoId, markerKey)) return 0;
  const sessions = legacySessions(repoRoot);
  return withControlPlaneTransaction(controllerHome, (database) => {
    const marker = readControlPlaneRecordWithinTransaction<LegacyImportMarker>(database, IMPORT_NAMESPACE, repoId, markerKey);
    if (marker) return 0;
    let imported = 0;
    for (const session of sessions) {
      const key = recordKey(repoId, session);
      const current = readControlPlaneRecordWithinTransaction<BrowserSessionAuthorityEntry>(database, SESSION_NAMESPACE, SESSION_SCOPE, key);
...
function authorityEntries(controllerHome: string): BrowserSessionAuthorityEntry[] {
  return listControlPlaneRecords<BrowserSessionAuthorityEntry>(controllerHome, {
    namespace: SESSION_NAMESPACE,
    scope: SESSION_SCOPE,
...
function activeEntries(controllerHome: string, repoId: string, repoRoot: string): BrowserSessionAuthorityEntry[] {
  ensureLegacyBrowserSessionsImported(controllerHome, repoId, repoRoot);
  return authorityEntries(controllerHome)
...
export function saveBrowserSession<T extends BrowserSessionAuthoritySession>(
  ...
  const key = recordKey(repoId, session);
  return withControlPlaneTransaction(controllerHome, (database) => {
    const current = readControlPlaneRecordWithinTransaction<BrowserSessionAuthorityEntry<T>>(database, SESSION_NAMESPACE, SESSION_SCOPE, key);
...
export function findBrowserSession<T extends BrowserSessionAuthoritySession>(
  ...
  return activeEntries(controllerHome, repoId, repoRoot)
...
export function listBrowserSessions<T extends BrowserSessionAuthoritySession>(
  ...
  const entries = activeEntries(controllerHome, repoId, repoRoot);
...
export function listAllBrowserSessionsForRepository<T extends BrowserSessionAuthoritySession>(
  ...
  return activeEntries(controllerHome, repoId, repoRoot).map((entry) => entry.session as T);
}
...
export function tombstoneBrowserSession(
  ...
  ensureLegacyBrowserSessionsImported(controllerHome, repoId, repoRoot);
  const record = listControlPlaneRecords<BrowserSessionAuthorityEntry>(controllerHome, {
    namespace: SESSION_NAMESPACE,
    scope: SESSION_SCOPE,
    limit: MAX_AUTHORITY_RECORDS,
  }).find((candidate) => {
  ...
  return withControlPlaneTransaction(controllerHome, (database) => {
    const current = readControlPlaneRecordWithinTransaction<BrowserSessionAuthorityEntry>(database, SESSION_NAMESPACE, SESSION_SCOPE, record.key);
...
export function createSqliteBrowserSessionAuthority(): BrowserSessionAuthorityPort {
  return {
    ensureLegacyImported: (context, repoRoot) => ensureLegacyBrowserSessionsImported(context.controllerHome, context.repoId, repoRoot),
    save: (context, repoRoot, session) => saveBrowserSession(context.controllerHome, context.repoId, repoRoot, session),
    find: (context, repoRoot, sessionId) => findBrowserSession(context.controllerHome, context.repoId, repoRoot, sessionId),
    list: (context, repoRoot, options) => listBrowserSessions(context.controllerHome, context.repoId, repoRoot, options),
    listAll: (context, repoRoot) => listAllBrowserSessionsForRepository(context.controllerHome, context.repoId, repoRoot),
    tombstone: (context, repoRoot, sessionId) => tombstoneBrowserSession(context.controllerHome, context.repoId, repoRoot, sessionId),
  };
}
