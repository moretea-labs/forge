...
export const runtimeToolDefinitions: McpToolDefinition[] = [
  ...
  definition('rh_work', 'Preferred ChatGPT facade for bounded execution, optional Requirement authority, optional planning, controller ownership, and external SuperController launch. Requirement and Plan are not universal prerequisites: start explicit bounded work directly when safe, establish Requirement authority only when durable goal identity is useful, and use Plan only for real decomposition, dependency ordering, or replanning.', {
    ...
    capability_id: { type: 'string', description: 'Legacy-readable compatibility input. operation=repair maps schedule.delete:<scheduleId> to schedule_delete, controller.disposition:<disposition>:<relayScopeId> to controller_disposition, controller.round:<operation>:<authorityId>:<relayScopeId> to the same fenced relay lifecycle operation for frozen clients, and controller.authority.recover:<workId> to explicit user-authorized recovery of an exact direct or relay-bound Work authority after MCP transport rotation, and work.review:<decision>:<workId> to the canonical implementation-review handler for frozen clients (with reason used as the required review rationale).' },
    work_id: { type: 'string' },
