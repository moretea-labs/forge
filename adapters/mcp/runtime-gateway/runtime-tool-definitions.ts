...
export const runtimeToolDefinitions: McpToolDefinition[] = [
  ...
  definition('rh_work', 'Preferred ChatGPT facade for bounded execution, optional Requirement authority, optional semantic planning, mechanical continuation, and external controller launch. Requirement and Plan are not universal prerequisites: start explicit bounded work directly when safe, establish Requirement authority only when durable goal identity is useful, and use Plan only for real decomposition, decisions, progress, or replanning.', {
    repo_id: repoId,
    checkout_id: { type: 'string', description: 'Optional checkout identity for exact Work and Plan source selection.' },
    operation: { type: 'string', enum: [...RH_WORK_MODEL_OPERATIONS], description: 'Defaults to start. Requirement, Plan, and optional Work are the model-facing durable semantic records. Plan is model-authored working memory revised in place by expected_revision CAS; its items never own paths, checks, Work, scheduling, or acceptance. A Work may reference plan_id as provenance without plan_step_id or Plan approval. Mechanical continuation and safety remain separate.' },
    objective: { type: 'string' },
    capability_id: { type: 'string', description: 'Optional capability identity for an explicitly typed recovery/repair path: controller.authority.recover:<workId> rebinds one exact Work controller authority, controller.provider.recover:<workId> rearms a blocked provider round with fresh evidence, controller.current_conversation.enroll adopts the authenticated current ChatGPT turn as ControllerRound authority, and plan.step.retry:<workId> restores one zero-delta cancelled legacy Plan item. operation=repair also maps semantic.v1:<bounded-payload> to explicitly allowlisted forward-compatible facade operations for frozen clients, then re-enters the canonical Runtime handler. Historical carriers may still encode old PlanStep/accept-step operations, but those are compatibility transport only and are not part of the current model-facing Plan authority.' },
    work_id: { type: 'string' },
    ...
    outcome_observation: { type: 'object', description: 'Typed Work-specific OutcomeObservation draft for outcome_record. sourceWorkId/sourceRoundId are derived from exact Work semantic provenance and cannot be supplied by the caller.' },
    experience_draft: { type: 'object', description: 'Typed verified Work Experience draft for experience_record. Generic teaching/correction belongs in learning_record instead; source provenance is derived from the current Work semantic revision.' },
    ...
    controller_id: { type: 'string', description: 'Stable external controller identity for mechanical continuation compatibility.' },
    controller_type: { type: 'string', enum: ['chatgpt', 'codex', 'grok', 'claude', 'human'], description: 'Controller kind for mechanical continuation or launcher_start.' },
    session_id: { type: 'string', description: 'Provider/controller session provenance for bounded continuation compatibility; it is not semantic Work ownership.' },
    lease_ms: { type: 'number', description: 'Bounded mechanical continuation lease/launch reservation compatibility. It does not grant repository mutation or semantic Work authority.' },
    ...
    superseded_by: { type: 'string', description: 'Legacy compatibility only. Current model-facing replans revise the same stable plan_id with expected_revision; they do not create successor Plan authority.' },
    expected_files: { type: 'number', description: 'Advisory predicted scope size for the model\u2019s own planning. It never classifies the Work, selects a provider, or chooses an execution mode. Escape hatch: the caller must declare work_kind explicitly instead of relying on this hint.' },
    expected_changed_lines: { type: 'number', description: 'Advisory predicted scope size for the model\u2019s own planning. It never classifies the Work, selects a provider, or chooses an execution mode.' },
    mode: { type: 'string', enum: ['plan', 'scale'], description: 'Explicit durable Plan/Scale execution mode. plan_id supplies semantic planning provenance only; Scale coordinates parallel execution through concrete Work/Process/resource fences, never PlanStep path/check/acceptance authority.' },
    ...
    requires_external_effect: { type: 'boolean', description: 'Declares a non-repository side effect. When no work_kind is declared, a pure effect Work is admitted as local_effect/remote_effect; supplying a predicted scope size together with an external effect without work_kind fails closed (WORK_KIND_REQUIRED_FOR_EXTERNAL_EFFECT_WITH_PREDICTED_SCOPE).' },
    ...
    engineering_blocker: { type: 'object', description: 'Explicit semantic blocker classification for continue. same_root_cause remains conservative Design re-entry; same_root_cause_scope_extension preserves the current architecture authority and exact candidate; unrelated requires linked_work_id naming the Work that already owns the blocker (Forge never creates a child Work automatically).', properties: {
      blocker_id: { type: 'string' }, classification: { type: 'string', enum: ['same_root_cause', 'same_root_cause_scope_extension', 'unrelated'] }, rationale: { type: 'string' }, semantic_scope_keys: { type: 'array', items: { type: 'string' } }, linked_work_id: { type: 'string', description: 'Required when classification=unrelated. Exact existing Work id that owns the blocker; start that Work explicitly first.' },
