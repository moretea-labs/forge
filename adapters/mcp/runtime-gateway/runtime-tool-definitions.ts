...
export const runtimeToolDefinitions: McpToolDefinition[] = [
  ...
  definition('rh_work', 'Preferred ChatGPT facade for bounded execution, optional Requirement authority, optional planning, controller ownership, and external SuperController launch. Requirement and Plan are not universal prerequisites: start explicit bounded work directly when safe, establish Requirement authority only when durable goal identity is useful, and use Plan only for real decomposition, dependency ordering, or replanning.', {
    ...
    operation: { type: 'string', enum: [...RH_WORK_OPERATIONS], description: 'Defaults to start. learning_record persists model-identified advisory learning directly to Cognitive memory without requiring Work/ControllerRound; learning_feedback records bounded used/rejected retrieval feedback without creating lifecycle and never changes factual confidence. The model decides whether a reusable knowledge delta exists at all; zero learning is expected for routine work. User correction is only one source alongside execution-discovered controller_observation/system_inference. Forge validates scope/provenance/evidence and never infers the lesson. controller_disposition records ChatGPT\'s explicit end-of-round continue/wait decision; Forge never infers the semantic next step. requirement_create establishes optional durable Requirement authority only and never implies plan_create. requirement_promote_candidate explicitly validates a Workspace Cognitive requirement-candidate from the selected repository semantic Project and enters the same canonical Requirement admission authority; candidate learning never calls it automatically. requirement_continue records an explicit semantic continue for a Requirement waiting_for_user and resumes it to active without creating a new Work or Plan. Plan-bound starts require plan_id and plan_step_id, but Plan is a late semantic commitment for real decomposition/replanning rather than a universal complex-task wrapper. plan_accept_step records explicit Controller semantic acceptance after machine delivery leaves a step validating. schedule_* manages Work-bound continuation, bounded browser observation, and explicit standalone browser keepalive.' },
    objective: { type: 'string' },
    capability_id: { type: 'string', description: 'Legacy-readable compatibility input. operation=repair maps semantic.v1:<bounded-payload> only to an explicitly allowlisted forward-compatible semantic facade operation, including learning_record for clients whose frozen schema predates that operation and then re-enters its canonical Runtime handler; current allowlist supports requirement_create, plan_create obligation continuity, and bounded start technical admission fields (work_kind, engineering_preconditions, paired controller_authority_id/relay_scope_id) while requirement_id remains an explicit stable field. Existing compatibility forms also map schedule.delete:<scheduleId>, controller.current_conversation.enroll for frozen clients that must elect current-conversation unattended continuation through the canonical controller_disposition handler, controller.disposition:<disposition>:<relayScopeId> for live-owner rounds or controller.disposition:<disposition>:<authorityId>:<relayScopeId> for exact terminal closure, controller.round:<operation>:<authorityId>:<relayScopeId> including plan_accept_step, controller.authority.recover:<workId>, plan.step.retry:<workId>, and work.review:<decision>:<workId> without creating alternate lifecycle or persistence authority.' },
    work_id: { type: 'string' },
    ...
    requirement_id: { type: 'string', description: 'Stable Requirement identity for requirement_get/requirement_revise/requirement_create/requirement_continue or optional semantic context link for Work/Plan.' },
    expected_revision: { type: 'number', minimum: 1, description: 'Expected current semantic revision for requirement_revise, plan_revise, or work_revise. Stale writers fail and return current state; Forge never performs an implicit merge.' },
    work_state: { type: 'string', enum: ['open', 'completed', 'cancelled'], description: 'Thin semantic Work state for work_revise. Execution failure/running/blocked/review/delivery state is mechanical and does not expand this enum.' },
    ...
    requirement_revision: { type: 'number', minimum: 1, description: 'Optional Requirement semantic revision provenance used by plan_revise as requirement basis or by work_revise as Work provenance. Later Requirement revisions are advisory staleness only and never automatically rebind or block execution.' },
    plan_revision: { type: 'number', minimum: 1, description: 'Optional Plan semantic revision provenance referenced by work_revise; later Plan revisions do not automatically rebind or block Work.' },
    ...
    requirement_state: { type: 'string', enum: ['open', 'completed', 'cancelled'], description: 'Thin semantic Requirement state for requirement_revise. Legacy planned/active/waiting runtime states project as open during migration.' },
    requirement_candidate_id: { type: 'string', description: 'Exact Cognitive memory id for operation=requirement_promote_candidate. Forge resolves Workspace scope from the selected repository ProjectPlacement, validates the advisory requirement-candidate and its promoted source/evidence, and binds provenance atomically to the canonical Requirement.' },
    ...
    enroll_current_conversation: { type: 'boolean', description: 'For controller_disposition=continue_immediately, adopt the authenticated current ChatGPT turn into ControllerRound authority, bind the exact browser-observed current conversation, and enroll the existing Workflow Supervisor task before release. Conversation/task/effect identities are never caller supplied.' },
    ...
    outcome_observation: { type: 'object', description: 'Typed Work-specific OutcomeObservation draft for outcome_record. sourceWorkId/sourceRoundId are derived from the exact current Controller authority and cannot be supplied by the caller.' },
    experience_draft: { type: 'object', description: 'Typed verified Work Experience draft for experience_record. Generic teaching/correction belongs in learning_record instead; source Work/Round authority is derived from the current Controller claim.' },
    ...
    plan_items: { type: 'array', maxItems: 100, items: { type: 'object', required: ['id', 'objective'], additionalProperties: false, properties: { id: { type: 'string', maxLength: 200 }, objective: { type: 'string', maxLength: 2000 }, dependencies: { type: 'array', maxItems: 100, items: { type: 'string', maxLength: 200 } } } }, description: 'Thin model-authored Plan items for plan_revise. Items are descriptive working memory only and never own paths, checks, Work, scheduling, or acceptance.' },
    plan_steps: { type: 'array', items: { type: 'object' }, description: 'Legacy PlanContract execution steps for plan_create compatibility; not the target Thin Plan semantic model.' },
