...
export const RUNTIME_LIFECYCLE_INVENTORY: readonly RuntimeLifecycleClassDefinition[] = Object.freeze([
  ...
  lifecycle({
    ...
    recoverySemantics: 'Active/nonterminal Jobs are protected. Terminal cleanup is fenced by the existing per-Job lock, removes only Job-owned record/request/worker-receipt/per-job-event state, preserves the global audit ledger, and never replays an accepted external effect.',
    closureStatus: 'existing_bounded', evidencePaths: ['src/runtime/execution/jobs/store.ts','src/runtime/control-plane/global-scheduler/maintenance.ts','tests/runtime/runtime-cleanup.test.ts'],
  }),
  lifecycle({
    ...
    retentionCapacity: 'A Process check receipt has no separate mutable store: it is deterministically derived from one terminal Process record, copied into Work VerificationRecord when consumed, and its source Process is bounded by the 7-day/500-terminal-record per-repository Process GC policy. Reusable check evidence may outlive the age horizon but not the 500-record capacity bound.',
    cleanupAuthority: 'Process GC owns the physical terminal Process/log source; Work lifecycle owns any accepted receipt copy embedded in VerificationRecord, so cleanup never needs a second receipt authority.',
    recoverySemantics: 'Missing Process evidence cannot synthesize a receipt; accepted Work-bound copies remain identity/fingerprint checked, and changed or missing authoritative evidence becomes stale/replay-gap evidence rather than synthetic success.',
    closureStatus: 'existing_bounded', evidencePaths: ['src/runtime/execution/process-runtime/check-receipt.ts','src/runtime/execution/process-runtime/gc.ts','packages/kernel/work/domain/types.ts','src/runtime/control-plane/execution/verification-evidence.ts'],
  }),
  lifecycle({
