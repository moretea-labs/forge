...
import { OPERATIONAL_TARGET_DEFINITIONS, type OperationalActionId, type OperationalMetricId, type OperationalObservation, type OperationalOutcomeClass, type OperationalTargetId } from '../../../packages/kernel/memory/api/index';
import type { ExecutionJob } from '../execution/jobs/types';

export interface OperationalExtractionIdentity {
...
function checkOutcomeClass(receipt:ProcessCheckReceiptEvidence):OperationalOutcomeClass{
  if(receipt.status==='cancelled')return'cancelled';if(receipt.status==='timed_out')return'timeout';if(receipt.status==='failed')return'unknown';if(receipt.status==='passed')return'executed_success';return'unknown';
}
...
export function extractExecutionJobShadowObservation(job:ExecutionJob,identity:OperationalExtractionIdentity):OperationalObservation{
  const outcomeClass=jobOutcomeClass(job),observedAt=job.finishedAt??job.updatedAt,startAt=job.startedAt??job.queuedAt;
  return{schemaVersion:1,observationId:`execution_job:${job.jobId}:${job.revision}:${identity.targetId}:${identity.actionId}:${identity.metricId}`,signalId:'execution_job.terminal_mechanical',targetId:identity.targetId,actionId:identity.actionId,metricId:identity.metricId,scope:identity.scope,source:{schema:'execution_job/v1',sourceId:job.jobId,eventKind:job.status,evidenceRef:`execution_job:${job.jobId}:${job.revision}`,requestId:job.requestId,observedAt,retainedUntil:identity.retainedUntil},attribution:{kind:identity.consumerRequestId===job.requestId?'exact':'ambiguous',consumerId:OPERATIONAL_TARGET_DEFINITIONS[identity.targetId].consumerId,requestId:identity.consumerRequestId,sourceId:job.jobId},outcomeClass,value:valueForMetric(identity.metricId,outcomeClass,elapsedMs(startAt,observedAt)),compatibility:{operation:job.payload.operation,environment:identity.environmentFingerprint}};
}
export function extractProcessCheckShadowObservation(receipt:ProcessCheckReceiptEvidence,identity:OperationalExtractionIdentity):OperationalObservation{
  const outcomeClass=checkOutcomeClass(receipt),requestId=receipt.requestId??'';
  return{schemaVersion:1,observationId:`process_check:${receipt.receiptId}:${identity.targetId}:${identity.actionId}:${identity.metricId}`,signalId:'process_check.terminal_mechanical',targetId:identity.targetId,actionId:identity.actionId,metricId:identity.metricId,scope:identity.scope,source:{schema:'process_check_receipt/v1',sourceId:receipt.receiptId,eventKind:receipt.status,evidenceRef:`process_check_receipt:${receipt.receiptId}`,requestId,observedAt:receipt.finishedAt,retainedUntil:identity.retainedUntil},attribution:{kind:requestId.length>0&&identity.consumerRequestId===requestId?'exact':'ambiguous',consumerId:OPERATIONAL_TARGET_DEFINITIONS[identity.targetId].consumerId,requestId:identity.consumerRequestId,sourceId:receipt.receiptId},outcomeClass,value:valueForMetric(identity.metricId,outcomeClass,elapsedMs(receipt.startedAt,receipt.finishedAt)),compatibility:{operation:`check:${receipt.checkId}`,environment:receipt.checkEnvironmentFingerprint??identity.environmentFingerprint}};
}
