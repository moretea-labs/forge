...
export function buildControllerRoundRelayPrompt(
  ...
  return [
    ...
    record.authorityId
      ? `This round's durable controller authority is controller_authority_id=${record.authorityId}. Pass this exact controller_authority_id together with relay_scope_id=${record.relayScopeId} on controller_claim, continue, finalize, stop, and controller_release. If the current frozen MCP schema omits controller_authority_id or relay_scope_id, pass the same opaque authority unchanged as session_id; Forge binds it to this exact Work's durable relay scope and still rejects another Work/conversation's authority. The opaque authority survives MCP transport/Runtime rotation within this round and rotates before a successor round.`
