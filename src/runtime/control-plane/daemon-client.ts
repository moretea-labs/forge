...
/**
 * Transitional read-only shape for callers that have not yet renamed their
 * diagnostics. It is not a Daemon lifecycle authority and cannot start,
 * replace, or restart any process.
 */
...
export function readControllerDaemonStatus(controllerHome: string): ControllerDaemonStatus {
  ...
  const status: ControllerDaemonStatus['status'] = observation.ready
    ? 'ready'
    : observation.running
      ? 'starting'
      : snapshot
