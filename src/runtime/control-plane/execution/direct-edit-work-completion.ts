...
/**
 * Historical failed-delivery recovery has no reachable controller_claim once
 * the Work is terminal. Preserve active-owner precedence, but when the exact
 * reviewed-recovery predicate holds and no owner remains, the durable
 * WorkHandle principal is the only prior ownership proof that may authorize
 * the same authenticated principal. Different principals and every other
 * terminal Work remain fail-closed.
 */
