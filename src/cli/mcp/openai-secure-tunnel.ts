...
export function parseOpenAiSecureTunnelRuntimeStatus(
  ...
  const identityMatches = value.tunnel_id === expected.tunnelId;
  const endpointMatches = tunnelRuntimeProfileTargetsEndpoint(value.profile_path, expected.mcpServerUrl);
  const ok = running && healthy && ready && identityMatches && endpointMatches;
  const mismatches: string[] = [];
  if (!identityMatches && value.tunnel_id) mismatches.push(`tunnel id mismatch (${value.tunnel_id})`);
