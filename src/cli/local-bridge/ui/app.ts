...
let data: ConsoleData | undefined; let busy=false; let selectedCapability=''; let loadError='';
...
function view(): string { if(!data)return loadError?`<div class="boot-state"><strong>Forge console unavailable</strong><div>${esc(loadError)}</div><button class="btn" data-refresh>Retry</button></div>`:'<div class="boot-state">正在读取 Forge 配置…</div>'; switch(route()){case'work':return renderWork(data);case'automations':return renderAutomations(data);case'capabilities':return renderCapabilities(data,selectedCapability);case'repositories':return renderRepositories(data);case'settings':return renderSettings(data);case'system':return renderSystem(data);default:return renderOverview(data);} }
function render(){root!.innerHTML=data?shell(view()):view();bind();}
async function refresh(){ if(busy)return; busy=true; loadError=''; render(); try{ const [commandCenter,work,automations,automationSettings,connector]=await Promise.all([api.commandCenter(),api.work(),api.automations(),api.automationSettings().catch(()=>({})),api.connector().catch(()=>({}))]); data={commandCenter,work,automations,automationSettings,connector}; }catch(error){ if(!data)loadError=error instanceof Error?error.message:String(error); else alert(error instanceof Error?error.message:String(error)); }finally{busy=false; render();} }
