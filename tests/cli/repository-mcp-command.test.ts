...
describe("repository MCP command tools", () => {
  ...
  test("finish workflow preserves unrelated dirty target changes and lets Git guard overlaps", async () => { const workspace = mkdtempSync(join(tmpdir(), "forge-structured-git-dirty-target-")); const controllerHome = join(workspace, "controller-home"); const repoRoot = join(workspace, "sample-repo"); try { mkdirSync(controllerHome, { recursive: true }); mkdirSync(repoRoot, { recursive: true }); git(repoRoot, ["init", "-b", "main"]); git(repoRoot, ["config", "user.name", "Forge Test"]); git(repoRoot, ["config", "user.email", "forge-test@example.com"]); writeFileSync(join(repoRoot, "README.md"), "hello\n"); writeFileSync(join(repoRoot, "local.txt"), "base\n"); git(repoRoot, ["add", "."]); git(repoRoot, ["commit", "-m", "init"]); const repository = registerRepository({ path: repoRoot, controllerHome, defaultBranch: "main" }); git(repoRoot, ["switch", "-c", "feature/dirty-switch"]); writeFileSync(join(repoRoot, "local.txt"), "dirty-before-switch\n"); const switchBlocked = await json(callRepositoryTool(controllerHome, "repository_git_finish_workflow", { repo_id: repository.repoId, feature_branch: "feature/dirty-switch", target_branch: "main" })); expect(switchBlocked.finish.error.code).toBe("GIT_WORKTREE_NOT_CLEAN"); expect(spawnSync("git", ["-C", repoRoot, "branch", "--show-current"], { encoding: "utf8" }).stdout.trim()).toBe("feature/dirty-switch"); git(repoRoot, ["restore", "local.txt"]); git(repoRoot, ["switch", "main"]); git(repoRoot, ["branch", "-D", "feature/dirty-switch"]); git(repoRoot, ["switch", "-c", "feature/dirty-target"]); writeFileSync(join(repoRoot, "README.md"), "hello\nfeature\n"); git(repoRoot, ["add", "README.md"]); git(repoRoot, ["commit", "-m", "feature"]); git(repoRoot, ["switch", "main"]); writeFileSync(join(repoRoot, "local.txt"), "local-dirty\n"); writeFileSync(join(repoRoot, "untracked.txt"), "keep\n"); const finish = await json(callRepositoryTool(controllerHome, "repository_git_finish_workflow", { repo_id: repository.repoId, feature_branch: "feature/dirty-target", target_branch: "main" })); expect(finish.finish.completed).toBe(true); expect(readFileSync(join(repoRoot, "README.md"), "utf8")).toBe("hello\nfeature\n"); expect(readFileSync(join(repoRoot, "local.txt"), "utf8")).toBe("local-dirty\n"); expect(readFileSync(join(repoRoot, "untracked.txt"), "utf8")).toBe("keep\n"); git(repoRoot, ["add", "local.txt"]); git(repoRoot, ["commit", "-m", "preserve local change for overlap setup"]); git(repoRoot, ["switch", "-c", "feature/overlap"]); writeFileSync(join(repoRoot, "README.md"), "feature-overlap\n"); git(repoRoot, ["add", "README.md"]); git(repoRoot, ["commit", "-m", "overlap feature"]); git(repoRoot, ["switch", "main"]); writeFileSync(join(repoRoot, "README.md"), "local-overlap\n"); const blocked = await json(callRepositoryTool(controllerHome, "repository_git_finish_workflow", { repo_id: repository.repoId, feature_branch: "feature/overlap", target_branch: "main" })); expect(blocked.finish.completed).toBe(false); expect(blocked.finish.error.code).toBe("GIT_MERGE_FAILED"); expect(readFileSync(join(repoRoot, "README.md"), "utf8")).toBe("local-overlap\n"); } finally { await cleanupWorkspace([workspace, controllerHome, repoRoot]); rmSync(workspace, { recursive: true, force: true }); } });
