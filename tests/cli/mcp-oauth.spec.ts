...
describe('MCP OAuth durable state', () => {
  ...
  test('repairs a corrupt primary from a valid fallback instead of starting empty', () => {
    const fx = fixture();
    try {
  ...
  test('fails closed when corrupt primary has no valid fallback', () => {
    const fx = fixture();
    try {
      writeFileSync(fx.primary, '{"accessTokens":', 'utf8');
      expect(() => new McpOAuthTokenStore(fx.primary).load()).toThrow('MCP_OAUTH_STORE_CORRUPT');
    } finally { fx.cleanup(); }
  });

  test('revoking refresh token removes refresh and linked access durably', async () => {
    const fx = fixture();
    try {
      const store = new McpOAuthTokenStore(fx.primary);
      store.setTokenPair('access-a', authInfo('access-a'), 'refresh-a');
  ...
  test('authorization codes expire and remain bound to issuing client', async () => {
    const fx = fixture();
    let now = 1_000;
    try {
  ...
  test('pending authorization codes are bounded per client', async () => {
    const fx = fixture();
    let now = 2_000;
    try {
      const provider = createMcpOAuthProvider(new McpOAuthTokenStore(fx.primary), {
