...
describe('runtime command surface', () => {
  ...
  test('MCP command surface exposes no KeepAlive or restart lifecycle owner', () => {
    ...
    const ingressStateStart = supervisorTypes.indexOf('  ingress: {');
    const ingressStateEnd = supervisorTypes.indexOf('  control?:', ingressStateStart);
    const ingressStateBlock = supervisorTypes.slice(ingressStateStart, ingressStateEnd);
    expect(ingressStateBlock).toContain("state: 'running' | 'degraded' | 'stopped'");
    expect(ingressStateBlock).toContain('pid?: number');
    expect(ingressStateBlock).not.toContain('consecutiveFailures');
    expect(ingressStateBlock).not.toContain('lastHealthyAt');
    expect(ingressStateBlock).not.toContain('lastFailureAt');
    expect(ingressStateBlock).not.toContain('lastFailureDetail');
    expect(supervisorStateStore).toContain("const ingress: SupervisorState['ingress']");
