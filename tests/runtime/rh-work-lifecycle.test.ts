...
describe('rh_work managed lifecycle closure', () => {
  ...
  test('non-shadow continuation trigger reserves one launch while authenticated MCP retains Work ownership authority', async () => {
    ...
    expect(continuedPayload.data?.controllerSession?.controllerType).toBe('codex'); const transientCtx = { ...externalCtx, sessionId: `${externalSession}-next` }; const command = await (await import('../../src/cli/mcp/repository-tools')).callRepositoryTool(fx.controllerHome, 'repository_command_execute', { repo_id: fx.repository.repoId, checkout_id: executionSession?.activeCheckoutId, work_id: work.workId, command: ['bun', '-e', 'await Bun.sleep(250)'], detail_level: 'detail', return_handle_immediately: true }, transientCtx); expect((command?.structuredContent as { process?: { workId?: string } }).process?.workId).toBe(work.workId); await Bun.sleep(300); const patch = await (await import('../../src/cli/mcp/repository-tools')).callRepositoryTool(fx.controllerHome, 'repository_safe_patch_apply', { repo_id: fx.repository.repoId, checkout_id: executionSession?.activeCheckoutId, work_id: work.workId, purpose: 'Workflow-bound edit', allowed_paths: ['workflow-attribution.txt'], operations: [{ type: 'create', path: 'workflow-attribution.txt', content: 'owned\n' }] }, transientCtx); expect((patch?.structuredContent as { session?: { workId?: string } }).session?.workId).toBe(work.workId);
