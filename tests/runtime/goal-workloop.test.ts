...
describe('goal workloop engine', () => {
  ...
  test('keeps reconciled work reviewable when durable output exists or checkout remains active', () => {
    ...
    expect(getWorkContract(ctx.workStore, artifactWorkId)?.status).toBe('waiting_for_review');
    expect(getWorkContract(ctx.workStore, activeWorkId)?.status).toBe('waiting_for_review');
  });
