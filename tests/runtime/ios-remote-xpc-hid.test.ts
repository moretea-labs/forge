...
describe('RemoteXPC HID backend', () => {
  ...
  it('keeps ASCII, backspace, and modifier chords on the connected CoreDevice keyboard', () => {
    ...
    expect(source).toContain("'modifierKeyboardSource': 'connected_coredevice' if direct_keyboard_service is not None else 'none'");
    expect(source).toContain("result = await dispatch_text(hid, direct_keyboard_service, 'connected_coredevice', 0.0)");
    expect(source).not.toContain('isolated_modifier_keyboard');
  ...
  it('maps backspace through the direct built-in keyboard report path', () => {
    ...
    expect(source).toContain("result = await dispatch_text(hid, direct_keyboard_service, 'connected_coredevice', 0.0)");
    expect(source).toContain("raise RuntimeError('Connected CoreDevice keyboard service is unavailable')");
  });
  ...
  it('reuses the connected CoreDevice keyboard for Command-A and Command-V whole-field replacement', () => {
    const source = remoteXpcHidWorkerSourceForTest();
    expect(source).toContain("raise RuntimeError('Connected CoreDevice keyboard service is unavailable')");
    expect(source).toContain("result = await dispatch_text(hid, direct_keyboard_service, 'connected_coredevice', 0.0)");
  ...
  it('isolates pasteboard transport while keeping modifier chords on the connected keyboard and stages text before selecting existing content', () => {
    const source = remoteXpcHidWorkerSourceForTest();
    ...
    expect(source).not.toContain('async with UniversalHIDServiceService(modifier_rsd) as modifier_hid:');
  });
